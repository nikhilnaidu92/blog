...
import com.aexp.commercial.data.util.SFTPUtil;
...
public class DataFeedService {
  ...
  public DataFeedService(DemographicDetailRepository demographicDetailRepository,
                         ...
                         HiPEDUtil hiPEDUtil, ResourceLoader resourceLoader, SFTPUtil sftpUtil) {
  ...
  public void generateDemographicDataFile() {
    String fileName = sftpUtil.getFileName("/DEMOGRAPHIC_DATA_");
  ...
  public void generateEmailDataFile() {
    String fileName = sftpUtil.getFileName("/EMAIL_DATA_");
  ...
  public void populateDemographicSheet(Sheet sheet, List<DemographicDetailEntity> demographicDetails) {
    ...
    for (DemographicDetailEntity demographicDetailEntity : demographicDetails) {
      try {
        ...
        log.info("Processing Demographic Detail Application ID: {}", applicationId);
        ...
        log.error("Error generating demographic data for Application ID: {} due to: {}", applicationId, ex.getMessage(), ex);
      }
    }
  }
    ...
    try {
      ...
      for (int i = 0; i < poCount; i++) {
        if (ownershipData.getPrincipalOwners().size() > i) {
          PrincipalOwner owner = ownershipData.getPrincipalOwners().get(i);
          if (owner != null) {
  ...
  public void setPrincipalOwnerData(Row row, PrincipalOwner owner, int index) {
  ...
  public void populateEmailSheet(Sheet sheet, List<EmailRequestEntity> emailRequests) {
    ...
    for (EmailRequestEntity emailRequestEntity : emailRequests) {
      try {
        ...
        log.info("Processing Email RequestId ID: {}", requestId);
        Row row = sheet.createRow(rowIndex++);
        ...
        log.error("Error generating email data for Email Request ID: {} due to: {}", requestId, ex.getMessage(), ex);
      }
    }
  }
